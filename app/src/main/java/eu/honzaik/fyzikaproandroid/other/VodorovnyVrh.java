...
public class VodorovnyVrh {
    ...
    public VodorovnyVrh(float pocatecniRychlost, float pocatecniVyska){
    ...
    private ArrayList<String> generateXValues(int type){
        ...
        if(type == 0 || type == 2){
            ...
            String value = "";
            for(int i = 0; i <= pocetIteraci; i++){
                float x = pocatecniRychlost * (((float) i / (float) pocetIteraci) * celkovyCas);
    ...
    private ArrayList<Entry> generateYValues(int type){
        ...
        for(int i = 0; i <= pocetIteraci; i++){
            /////////////
            if(type == 0){
                float rychlostX = pocatecniRychlost;
                float rychlostY = zrychleni * (((float) i / (float) pocetIteraci) * celkovyCas);
            ...
            if(type == 1){ // trajektorie
                float y = pocatecniVyska - 0.5f * zrychleni * (float) Math.pow(((float) i / (float) pocetIteraci) * celkovyCas, 2);
                if (y < 0) y = 0;
                entry = new Entry(y, i);
            }
